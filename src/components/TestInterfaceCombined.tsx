...
import { Clock, ChevronLeft, ChevronRight, X, Check, ToggleLeft, ToggleRight } from "lucide-react";
...
export const TestInterfaceCombined = ({ 
  ...
  const resolveWebpImage = (imageUrl?: string | null) => {
    if (!imageUrl?.trim()) {
      return undefined;
    }
    ...
    const fileName = imageUrl.replace(/^\//, "").split("/").pop()?.replace(/\.[^.]+$/, "");
    return fileName ? `${imagePrefix}${fileName}.webp` : undefined;
  };
  ...
  const fetchAndMergeData = async () => {
    try {
      ...
      const transformedQuestions: Question[] = selectedQuestions.map((q, idx) => {
        // Format 4: 1200.json-style array with image and options
        if (q.options && Array.isArray(q.options) && !q.content && !q.choises) {
          ...
          return {
            id: idx + 1,
            text: q.question,
            image: resolveWebpImage(q.image),
        ...
        if (q.content && (q.content.uz_lat || q.content.uz_cyr || q.content.ru)) {
          ...
          return {
            ...
            image: resolveWebpImage(q.media_url),
            correctAnswer,
        ...
        if (q.choises && Array.isArray(q.choises)) {
          ...
          return {
            id: idx + 1,
            text: q.question,
            image: q.media?.exist && q.media?.name ? `${imagePrefix}${q.media.name}.png` : resolveWebpImage(q.image),
            correctAnswer: correctIndex + 1,
